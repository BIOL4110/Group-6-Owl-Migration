...
# You will first need your own acces token, go to https://osf.io/settings/tokens/ to sign up and get one, then run:
# osf_auth("your_code")
...
```
# This final chunk is now use to dowload the temprature data into a dataframe that is dowloaded onto a directory of your choice.
# Variables that needed to be altred are lat_indices and lon_indices (this was to specify specific coordinates of the grid).
# Other variables that can be altered are mentioned in the chunk itself in eachsections annotation
```{r}
...
lat_indices <- which(lat >= 50 & lat <= 55)
lon_indices <- which(lon >= -80 & lon <= -50)
...
# Filter out any rows that are past the specified end year, this value will be your last year
...
# Chnage ID in the code to the grid ID 
write.csv(export_df, file = "Temp_ID.csv", row.names = FALSE)
```
